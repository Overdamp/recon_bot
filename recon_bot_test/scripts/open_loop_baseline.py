...
class OpenLoopBaseline(Node):
    def __init__(self):
        super().__init__('open_loop_baseline')
        
        # --- Control Publishers ---
        self.publisher_ = self.create_publisher(Twist, '/cmd_vel', 10)
        
        # --- Logging Setup ---
        self.declare_parameter('log_dir', os.path.expanduser('~/recon_bot_logs/baseline'))
        ...
        self.filename = os.path.join(self.log_dir, f'baseline_test_{timestamp}.csv')
        ...
        self.get_logger().info(f'Logging Baseline Data to: {self.filename}')
        ...
        self.get_logger().info('Starting Baseline Test (Raw Odom vs SLAM)')
...
def plot_results(filename):
    ...
    try:
        ...
        plt.plot(odom_x, odom_y, label='Baseline (Wheel Odom)', color='blue', linestyle='--')
        plt.scatter(0, 0, color='blue', marker='o', label='Start')
        
        plt.title('Baseline Experiment: Wheel Odom vs Ground Truth')
